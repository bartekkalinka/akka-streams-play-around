...
object SwitchingTick {
  ...
  def apply(firstInterval: FiniteDuration, secondInterval: FiniteDuration, switchingInterval: FiniteDuration): Source[Unit, NotUsed] = {
    ...
    Source.fromGraph(GraphDSL.create() { implicit builder: GraphDSL.Builder[NotUsed] =>
      ...
      broadcastNode.out(0).scan(false)(neg).expand(a => Transformations.oneAndRepeatIterator(a, a)).outlet ~> zipNode1.in1
      val zipNode2 = zipWithNode[Boolean]
      secondTick ~> zipNode2.in0
      broadcastNode.out(1).scan(true)(neg).expand(a => Transformations.oneAndRepeatIterator(a, a)).outlet ~> zipNode2.in1
...
object Ticks {
  ...
  def switchingTick: Source[Unit, NotUsed] = SwitchingTick(0.05 seconds, 0.7 seconds, 3 seconds)
}

object Counter {
  def apply[A](tick: Source[Unit, A]): Source[Long, NotUsed] = Transformations.zipWithTick(tick, Source(Stream.iterate(0L)(_ + 1)))
}

object Transformations {
  def zipWithNode[A](implicit builder: GraphDSL.Builder[NotUsed]): FanInShape2[Unit, A, A] =  {
  ...
  def zipWithTick[A, B](tick: Source[Unit, B], toZip: Source[A, NotUsed]): Source[A, NotUsed] =
    Source.fromGraph(GraphDSL.create() { implicit builder: GraphDSL.Builder[NotUsed] =>
      import GraphDSL.Implicits._
  ...
  def conflateToLast[A](tick: Source[Unit, Cancellable], source: Source[A, NotUsed]): Source[A, NotUsed] = {
    val takeLast: Source[A, NotUsed] = source.conflate((acc, elem) => elem)
    zipWithTick(tick, takeLast)
  }

  def lastElemOption[A](tick: Source[Unit, Cancellable], source: Source[A, NotUsed]): Source[Option[A], NotUsed] = {
    val pipedSource = source.transform(() => new LastElemOption[A]())
    zipWithTick(tick, pipedSource)
  }

  def oneAndRepeatIterator[A](a: A, repeat: A): Iterator[A] = Iterator(a) ++ Iterator.continually(repeat)
}
...
object CountersDemo {
  ...
  def mediumDropped: Source[Long, NotUsed] = conflateToLast(mediumTick, Counter(fastTick))

  def mediumThroughFast: Source[Option[Long], NotUsed] = lastElemOption(fastTick, Counter(mediumTick))

  def switchingThroughFast: Source[Option[Long], NotUsed] = lastElemOption(fastTick, Counter(switchingTick))
